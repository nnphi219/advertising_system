    ...
    return (
      <div>
        <div className="adsarea--header">
          <h2 className="adsarea--header-title">Dịch vụ quảng cáo</h2>
        </div>
        <div>
          <NavLink to={this.props.linkTo} className="btn btn-primary">Tạo dịch vụ quảng cáo</NavLink>
        </div>
      </div>
    );
  }
...
class AdsArea extends Component {
  ...
  handleEditClick(event) {
    window.location.href = UrlRedirect.AdsAreaEditor + `/${event.target.name}`;
  }

  handleDeleteClick(event) {
    let selectedItemId = event.target.name;
    let selectedItem = this.state.tbodyAdsAreas.find((content) => content._id === selectedItemId);
    this.setState({
      ShowDeletePopup: !this.state.ShowDeletePopup,
      SelectedItemId: selectedItemId,
      selectedItemValue: selectedItem.ten_hien_thi
    });
  }
          ...
          <div id="page-wrapper" onKeyDown={this.onKeyDown}>
            <div className="row">
              <div style={{ marginLeft: "10px" }}>
                <div>
                  <AdsAreaHeader
                    showCreatorPopup={this.handleShowCreatorPopup}
                    linkTo={UrlRedirect.AdsAreaCreator}
                  />
                </div>
                <AdsAreaContents
                  tbodyAdsAreas={tbody}
...
class AdsAreaManagement extends Component {
  render() {
    return (
      <BrowserRouter>
        <div>
          <Route exact={true} path={UrlRedirect.AdsArea} component={AdsArea} />
          <Route exact={true} path={UrlRedirect.AdsAreaCreator} component={AdsAreaCreator} />
          <Route exact={true} path={UrlRedirect.AdsAreaEditor + "/:id"} component={AdsAreaEditor} />
        </div>
      </BrowserRouter>
    );
  }
...
export default AdsAreaManagement;