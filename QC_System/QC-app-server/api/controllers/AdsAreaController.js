    ...
    AdsArea.find({ nguoi_tao: username }, function (err, adsAreas) {
        if (err) {
            ...
            let adsareaIdInfo = adsAreas.map((adsArea) => {
                return _.pick(adsArea, ['_id', 'ma_dich_vu', 'ten_hien_thi', 'loai_trang_ap_dung', 'loai_quang_cao', 'tin_rao_api', 'so_luong_chia_se_vung', 'so_luong_tin_toi_da', 'url_hinh_anh_vung_quang_cao']);
            });
            next(adsareaIdInfo);
        }
    });
